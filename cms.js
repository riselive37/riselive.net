    ...
    contents.forEach(item => {
        ...
        const categoryName = item.category
            ? (Array.isArray(item.category)
                ? (item.category[0]?.name || item.category[0] || 'Work')
...
const renderNewsDetailContent = (id, data) => {
    ...
    if (data) {
        ...
        // Not found
        detailContainer.innerHTML = '<div class="news-detail-content"><p>記事が見つかりませんでした。</p><div class="news-detail-actions"><a href="./" class="rl-btn rl-btn-outline">一覧に戻る</a></div></div>';
    ...
    detailContainer.innerHTML = `
        <article class="news-detail-content fade-in-up visible">
            <header class="news-header">
                <a href="./" class="rl-text-link">News一覧へ戻る</a>
                <div class="news-meta">
                    <time>${date}</time>
            ...
            <div class="news-detail-actions">
                <a href="./" class="rl-btn rl-btn-outline">一覧に戻る</a>
                <a href="../contact/" class="rl-text-link">この記事に近い内容を相談する</a>
            </div>
        </article>
    `;
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (newsId && document.getElementById('newsDetail')) {
        ...
        // Fetch only published CMS entries. Unknown IDs render the safe not-found state.
        if (CMS_CONFIG.serviceDomain !== 'YOUR_SERVICE_DOMAIN') {
        ...
        btn.addEventListener('click', () => {
            ...
            if (filterValue === 'All') {
                ...
                const filtered = allWorksData.filter(item => {
                    const categoryName = item.category
                        ? (Array.isArray(item.category)
                            ? (item.category[0]?.name || item.category[0] || '')
                            : (item.category.name || item.category))
                        : '';
...
const initPagination = (containerId, perPage = 10) => {
    ...
    const renderControls = () => {
        ...
        for (let i = 1; i <= totalPages; i++) {
            ...
            btn.href = '#';
            btn.textContent = i;
            btn.className = i === currentPage ? 'pagination-link is-active' : 'pagination-link';
